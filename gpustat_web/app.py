...
DEFAULT_GPUSTAT_COMMAND = "gpustat+ -s --color --gpuname-width 30 --show-power"
    ...
    *,
        ...
        async with asyncssh.connect(hostname, port=port, **conn_kwargs) as conn:
            ...
            while True:
                ...
                if result.exit_status != 0:
                    cprint(
                        f"[{now} [{hostname:<{L}}] Error, exitcode={result.exit_status}",
                        color="red",
                    )
                    cprint(_str(result.stderr or ""), color="red")
                    stderr_summary = _str(result.stderr or "").split("\n")[0]
                    context.host_set_message(
                        hostname,
                        colored(
                            f"[exitcode {result.exit_status}] {stderr_summary}", "red"
                        ),
                    )
                else:
                    if verbose:
                        cprint(
                            f"[{now} [{hostname:<{L}}] OK from gpustat "
                            f"({len(_str(result.stdout or ''))} bytes)",
                            color="cyan",
                        )
                    # update data
    ...
    while True:
        try:
            ...
            context.host_set_message(
                hostname, colored(f"Timeout after {timeout} sec", "red")
            )
        except (
            ...
            context.host_set_message(
                hostname, colored(f"{type(e).__name__}: {e}", "red")
            )
        ...
        cprint(
            f"[{hostname:<{L}}] Disconnected, retrying in {poll_delay} sec...",
            color="yellow",
        )
        await asyncio.sleep(poll_delay)
...
def make_static_handler(content_type: str):
    ...
    async def handler(request: web.Request):
        ...
        body = render_gpustat_body(mode=content_type, full_html=full, nodes=nodes)
...
def create_app(
    *,
    ...
    aiojinja2.setup(
        app, loader=jinja2.FileSystemLoader(os.path.join(__PATH__, "template"))
    )
...
def main():
    ...
    parser.add_argument(
        "hosts", nargs="*", help="List of nodes. Syntax: HOSTNAME[:PORT]"
    )
